...
function setComposerDraft(prompt: string): void {
...
function openModal(): void {
  ...
  overlay.style.cssText = [
    ...
    'padding:24px', 'background:rgba(20,24,32,.28)', 'backdrop-filter:blur(3px)',
  ].join(';')
  const panel = document.createElement('section')
  panel.style.cssText = [
    'width:min(860px,100%)', 'max-height:min(760px,calc(100vh - 48px))', 'overflow:auto',
    'padding:24px', 'border-radius:20px', 'background:Canvas', 'color:CanvasText',
    'box-shadow:0 24px 70px rgba(0,0,0,.22)', 'font:inherit',
  ].join(';')
  ...
  for (const workflow of workflows) {
    ...
    use.addEventListener('click', () => { setComposerDraft(workflow.prompt); closeModal() })
  ...
  panel.querySelector<HTMLButtonElement>('[data-close]')?.addEventListener('click', closeModal)
...
function attach(): void {
  ...
  launcher.style.cssText = [
    'display:block', 'width:min(520px,calc(100% - 32px))', 'margin:12px auto 0', 'padding:10px 14px',
    'border:1px solid rgba(255,185,148,.7)', 'border-radius:12px', 'background:rgba(255,185,148,.16)',
    'color:inherit', 'font:600 14px/20px inherit', 'cursor:pointer', 'text-align:left',
  ].join(';')
...
export function mountResearchLauncher(): () => void {
  attach()
  const observer = new MutationObserver(attach)
  observer.observe(document.body, { childList: true, subtree: true })
  return () => { observer.disconnect(); document.getElementById(launcherId)?.remove(); closeModal() }
}
