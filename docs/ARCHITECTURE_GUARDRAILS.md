...
- 菜单、弹窗、输入、论文阅读器和插件安装确认拥有更高交互优先级。
...
- 猫咪、论文库和插件市场分别拥有独立状态边界，任何模块故障都不得污染 DSH 原生会话状态。
- 论文导入、索引与插件安装任务必须可恢复；刷新或重试不得重复导入、损坏笔记或留下幽灵插件。
- 插件安装、升级和卸载操作使用稳定 job id 与明确的回滚状态。
...
- 影响启动、插件加载、论文文件/索引/笔记、会话上下文桥接、插件安装或自动更新的变更属于高风险，必须增加失败路径验证。
